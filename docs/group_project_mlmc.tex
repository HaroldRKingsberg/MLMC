...
\section{Standard Monte Carlo for Euler-Maruyama Path Simulations}
    ...
    [xxx elaborate on discretization error and statistical error]    
...
\section{Multilevel Monte Carlo for Path Simulations}
	...
	Closely following \cite{giles08}, we denote $P=P(S(T))$ as the payoff based on $S(T)$, whose expectation $E[P]$ is the quantity we are interested in estimating. Let $\widehat{P_l}$ be the approximation of $P$ using \textit{one single path} only, with the step size $h_l$ corresponding to the MLMC level $l$ ($l=0,1,2,...L$, with $L$ being the finest level). We note that $E[\widehat{P_l}] \neq E[P]$, the difference between them being the discretization error, or the bias from using $E[\widehat{P_l}]$ to approximate $E[P]$. In general, the discretization error of $E[\widehat{P_l}]$ depends most on the step size $h_l$ (instead of on the number of independent paths generated). Existing theorems indicate that Euler-Maruyama discretization has weak convergence of order one and strong convergence of order one half, provided that the drift function $\mu(\cdot)$ and the diffusion function $\sigma(\cdot)$, as well as the payoff function $\P(\cdot)$ satisfy certain conditions (\cite{kloeden99}). The orders of convergence of Euler-Maruyama are expressed as follows \cite{higham16}:
	\begin{align*}
