...
\section{Standard Monte Carlo for Euler-Maruyama Path Simulations}
	...
	We can further examine the error by separating it into a discretization error and a statistical error. Closely following \cite{giles08}, we denote $P=P(S(T))$ as the payoff based on $S(T)$, whose expectation $E[P]$ is the quantity we are interested in estimating. In practice, we cannot simulate $S(T)$ and $P$, but instead rely on simulating small incremental time steps of size $h$ each. Let $\widehat{S(t)}$ be the approximation of $S(t)$ via simulating with time step $h$, and \widehat{P_h} be the associated approximation of $P$, where the subscript $h$ is to emphasize the step size. Provided that conditions on $\mu(\cdot)$ and $\sigma(\cdot)$ are met, the orders of convergence of Euler-Maruyama are expressed as follows \cite{higham15}:
	\begin{align*}
...
\section{Multilevel Monte Carlo for Path Simulations}
	...
	Following notations from \cite{giles08}, let $\widehat{P_l}$ be the approximation of $P$ using \textit{one single path} only with the step size $h_l$ corresponding to the MLMC level $l$ ($l=0,1,2,...L$, with $L$ being the finest level). We note that $E[\widehat{P_l}] \neq E[P]$, the difference between them being the discretization error, or the bias from using $E[\widehat{P_l}]$ to approximate $E[P]$. In general, the discretization error of $E[\widehat{P_l}]$ depends most on the step size $h_l$ (instead of on the number of independent paths generated). Existing theorems indicate that Euler-Maruyama discretization has weak convergence of order one and strong convergence of order one half, provided that the drift function $\mu(\cdot)$ and the diffusion function $\sigma(\cdot)$, as well as the payoff function $\P(\cdot)$ satisfy certain conditions (\cite{kloeden99}). 
	
	Provided that these conditions are met, then $E[\widehat{P_l} - P] = O(h_l)$ as $l\rightarrow \infty$. Our goal is use to $E[\widehat{P_L}]$ as the approximation (though biased) of $E[P]$. 
	...
	Discretization from step size and statistical errors from paths
